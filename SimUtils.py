...
import ROOT
import os, sys
from GeomUtils import GetListOfGeometries, SubstituteDimensions
...
def GetFilesInfo(card, options = []):
    ...
    optionsAvailable = ['jobs','g4','cluster','recon','fastlight']
    ...
    # Geometries to be looped over
    geoms = GetListOfGeometries(card.Get('Geometries')) if card.Has('Geometries') else [None]
        ...
        for pos in card.Get('LocationList').split(','):
            for isotope, ratio in zip(card.Get('DecayChainList').split(','), card.Get('BranchRatioList').split(',')):
                for geom in geoms:
                    ...
                    resultInfo['MacroModel'] = SubstituteDimensions(card.Get('MacroModel'),geom)
                    
                    fileNamePattern = card.Get('FileNamePattern')
                    fileNamePattern = fileNamePattern.replace('[ParentName]',card.Get('ParentName'))
                    fileNamePattern = fileNamePattern.replace('[Isotope]',isotope.replace(' ',''))
                    fileNamePattern = fileNamePattern.replace('[Location]',location)
                    fileNamePattern = fileNamePattern.replace('[Seed]',str(seed))                
                    fileNamePattern = fileNamePattern.replace('[Geometry]',geom['_label'] if card.Has('Geometries') else 'FixedGeometry')
                    ...
                    if 'fastlight' in options:
                        flNamePattern = fileNamePattern.replace('[OutputNamePrefix]',card.Get('FastlightOutputNamePrefix'))
                        flFullPath = card.Get('FastlightFullOutputPath')
                        if hasFileTopDir: flFullPath = flFullPath.replace('[FileTopDir]',card.Get('FileTopDir'))
                        resultInfo['FastlightFileName'] = '%s/%s.%s.root'%(flFullPath,flNamePattern,card.Get('FastlightTreeName'))
                    ...
                    if card.Has('UseScratchDir') and card.Get('UseScratchDir').lower() != 'no':
                        ...
                        scratchDir = scratchDir.replace('[Isotope]',isotope.replace(' ',''))
