...
def GetNentries(n,card):
    ...
    if card.Has('ReconstructNevents'):
        ...
        if optN.lower() == 'all':
            optN = n
            print(optN, n) #for tests
        else:
            optN = int(optN)
        n = min(n,optN)
    print("n events to reconstruct: ", n)
...
def CreateReconTree(treeName, treeTitle = 'Tree of reconstructed energy deposits'):

    global reconVars

    #tree = OutputRootFileFileName.Get(card.Get('ClusterTreeName'))
...
def LoadLXeDimensions():
    ...
    dimNames = ['LXeCenterX','LXeCenterY','LXeCenterZ', 
                'ActiveLXeRadius', 'ActiveLXeMinAxial', 'ActiveLXeMaxAxial',
                'FidVolRadius', 'FidVolMinAxial', 'FidVolMaxAxial']
...
def InFV(x,y,z):
    ...
    if (radial1**2 + radial2**2)**0.5 < globalArgs['FidVolRadius'] and globalArgs['FidVolMinAxial'] < axial and axial < globalArgs['FidVolMaxAxial']:
        return True
...
def GetStandoffDistance(x,y,z):
    ...
    if lXeSDdim:
        ...
        lXeRadius, lXeMinAx, lXeMaxAx = globalArgs['ActiveLXeRadius'], globalArgs['ActiveLXeMinAxial'], globalArgs['ActiveLXeMaxAxial']
...
#@profile
def ApplyReconstruction(tree, rtree):
...
if __name__ == "__main__":
    ...
    InputTree = ReadInputTree(card.Get('ClusterTreeName'))
    ReconTree = CreateReconTree(card.Get('ReconTreeName'))
    LoadLXeDimensions()

    ApplyReconstruction(InputTree, ReconTree)
