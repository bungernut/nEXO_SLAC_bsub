...
def LoadLXeDimensions():

    dimNames = ['LXeCenterX','LXeCenterY','LXeCenterZ', 'ActiveLXeRadius', 'ActiveLXeMinAxial', 'ActiveLXeMaxAxial']
  
    # if the dimensions of the active volume is defined in the card, then use those values.
    if all([card.Has(name) for name in dimNames]):
      print('LXe dimensions found in card.')
      for name in dimNames:
        globalArgs[name] = float(card.Get(name))
      return 
    print('LXe dimensions not found in card! Extracting from input root file...')
       
    # if not, deduce from geometry in G4 output file
    activeLXeDims = GeomUtils.ExtractLXeDimensions(globalArgs['InputRootFileName'])
    for name in dimNames:
      globalArgs[name] = activeLXeDims[name]
    return 

def InActiveLXe(x,y,z):

    card = globalArgs['Card']
    lXeCenterX, lXeCenterY, lXeCenterZ = globalArgs['LXeCenterX'], globalArgs['LXeCenterY'], globalArgs['LXeCenterZ']
    ...
    if (radial1**2 + radial2**2)**0.5 < globalArgs['ActiveLXeRadius'] and globalArgs['ActiveLXeMinAxial'] < axial and axial < globalArgs['ActiveLXeMaxAxial']:
        return True
...
def AreClose(x1,y1,z1,x2,y2,z2,d0):
  if abs(x1-x2) > d0 or abs(y1-y2) > d0 or abs(z1-z2) > d0:
    return False
  return Distance(x1,y1,z1,x2,y2,z2) < d0

def ClusterDeposits(Deps):

    global clusterVars
    card = globalArgs['Card']
    clusterDiameter = float(card.Get('ClusterDiameter'))
    while len(Deps):
        x1 = Deps[0][0]
        y1 = Deps[0][1]
        z1 = Deps[0][2]
        ...
        for element in range(1,len(Deps)):
            ...
            y2 = Deps[element][1]
            z2 = Deps[element][2]
            if AreClose(x1,y1,z1,x2,y2,z2,clusterDiameter):
                picked = True
                e1 = Deps[0][3]
                e2 = Deps[element][3]
                eNew = e1 + e2
...
def ApplyClustering(tree,clusterTree):
    ...
    #Sniperized way of accessing event data
    i=0
    for header in tree:
        
        if i%1000 == 0:    print('cluster i = ', i)
        
        ClearClusterVars()

        simevent = header.SimEvent
        clusterVars['EventVertexX'][0][0] = simevent.GenX()
        clusterVars['EventVertexY'][0][0] = simevent.GenY()
        clusterVars['EventVertexZ'][0][0] = simevent.GenZ()
        clusterVars['EventNumber'][0][0] = simevent.EventNumber()

        Deps = []

        for j in range(simevent.NumDeposits()):
            Dep = [simevent.Xpos()[j], simevent.Ypos()[j], simevent.Zpos()[j], simevent.EnergyDeposit()[j]]
...
'''
    for i in range(0, n):
        ...
        #clusterVars['EventVertexX'] = tree.GenX[0]
        clusterVars['EventVertexY'] = tree.GenY()
        clusterVars['EventVertexZ'] = tree.fGenZ
        clusterVars['EventNumber'] = tree.fEventNumber
        #print(tree.fNumDeposits)
        #print(float(tree.GenX()))
        Deps = []
        for j in range(tree.fNumDeposits):
        #for j in range(0, 100000):
            Dep = [tree.fXpos[j], tree.fYpos[j], tree.fZpos[j], tree.fEnergyDeposit[j]]
            if InActiveLXe(Dep[0],Dep[1],Dep[2]):
                Deps.append(Dep)
